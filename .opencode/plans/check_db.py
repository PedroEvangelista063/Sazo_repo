"""Check DB users and try to apply migrations."""
import asyncio
import asyncpg
...
async def main():
    dsn = "postgresql://role_api_reader:senha@localhost:5432/quero_comprar"
    conn = await asyncpg.connect(dsn)

    # Check roles
    rows = await conn.fetch(
        "SELECT rolname, rolsuper, rolcreaterole FROM pg_roles WHERE rolname IN ('postgres', 'role_api_reader', 'role_etl')"
    )
    print("Roles:")
    for r in rows:
        print(f"  {r['rolname']}: super={r['rolsuper']} createrole={r['rolcreaterole']}")

    # Check schema privileges
    row = await conn.fetchrow(
        "SELECT has_schema_privilege('role_api_reader', 'mart', 'CREATE') AS can_create"
    )
    print(f"role_api_reader CREATE on mart: {row['can_create']}")

    # Can I create the views with role_api_reader?
    try:
        await conn.execute(MIGRATION_34)
        print("Migration 34: OK")
    except Exception as e:
        print(f"Migration 34 FAILED: {e}")
