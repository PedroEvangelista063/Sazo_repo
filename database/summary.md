...
- **Migration 77 (nomenclatura DBA-friendly): APLICADA nos DOIS bancos (LOCAL + AIVEN, 2026-08-12)** com backups `backup_schema_77_pre_*` e `backup_schema_77_pre_aiven_*`. A 77 foi **reescrita** antes da aplicação: **wrappers de compatibilidade de 30 dias para as 9 funções renomeadas** (BLOCO 4.5, com DEFAULTs replicados — `p_uf DEFAULT NULL`, `p_dry_run DEFAULT TRUE`) + **guard de idempotência** no BLOCO 4 (2ª execução no-op nos 2 bancos) + **BLOCO 4.6: corpos chamadores reescritos para os nomes novos** (`validar_anomalia_preco` chama `classificar_preco_anomalia`; `consolidar_produtos_duplicados` chama `relatorio_normalizacao_produtos`) — wrappers puramente vestigiais. **Validação pós-aplicação (2 bancos)**: 10 nomes novos + 9 wrappers, MV intacta (177.485), 0 nulo/CINZA, trigger em runtime capturando preço anômalo R$99.999, defaults 0-arg OK, smoke PASS. **Consistência local×Aiven pós-sync**: idênticos (nomes novos 4=4, wrappers 1=1, `raw.preco_conab_uf` 1=1, MV 177.485=177.485). **Fase 3 agendada**: `database/fase3_drop_compat_77.sql` (guard temporal até 2026-09-30 + validações pré/pós-drop) — seguro pois corpos já usam nomes novos.
