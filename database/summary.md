...
- **Migration 77 (nomenclatura DBA-friendly): DECISÃO HOLD RESOLVIDA (2026-08-12)** — a 77 foi **reescrita** com **wrappers de compatibilidade de 30 dias para as 9 funções renomeadas** (novo BLOCO 4.5, replicando o padrão das views do BLOCO 5) + **guard de idempotência no BLOCO 4** (só renomeia se o novo nome ainda não existir com a mesma identidade — evita re-renomear wrappers numa 2ª execução). **Evidência da revisão**: (1) catálogo vivo — `staging.trg_valida_anomalia_preco` chama `staging.fn_classificar_preco_anomalia` por nome no corpo e `staging.fn_consolidar_produtos_duplicados` chama `staging.fn_relatorio_normalizacao`; (2) zero referências em código vivo (Python/TS/sh); (3) **validada em transação ROLLBACK** no banco local (nada persistido): 16 renames + 9 wrappers + 6 views, **2ª execução idempotente**, trigger segue ativo apontando para `validar_anomalia_preco` (via OID), wrappers delegam em runtime. **Pendência**: aplicar local (homologação) + Aiven após backup completo + aprovação.
