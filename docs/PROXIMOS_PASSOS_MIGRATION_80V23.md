...
- [x] **Revisar a migration 77** (nomenclatura DBA-friendly) — **HOLD RESOLVIDO em 2026-08-12**: a 77 foi **reescrita com wrappers de compatibilidade de 30 dias para as 9 funções renomeadas** (BLOCO 4.5, padrão do BLOCO 5 das views) + guard de idempotência no BLOCO 4 (evita re-renomear wrappers numa 2ª execução). **Evidência da revisão**: (1) catálogo vivo mostra cross-references por nome em `staging.trg_valida_anomalia_preco` → `fn_classificar_preco_anomalia` e `staging.fn_consolidar_produtos_duplicados` → `fn_relatorio_normalizacao`; (2) zero referências em código vivo (Python/TS/sh); (3) **validada em transação ROLLBACK** no banco local (nada persistido): 16 renames + 9 wrappers + 6 views, **2ª execução idempotente sem erros**, trigger segue ativo apontando para `validar_anomalia_preco` (via OID), wrappers delegam em runtime (`fn_classificar_preco_anomalia(1,1,2026,1,100)` → `HARD_CAP_50_SEM_HISTORICO`, `_parse_conab_price('2,27')` → `2.2700`, `_gerar_batch_id()` → UUID, `fn_relatorio_normalizacao()` → 32 linhas). **Pendência restante**: aplicar local (gate de homologação) + Aiven, somente após backup completo + aprovação do time.
