...
- [x] **Revisar e aplicar a migration 77** (nomenclatura DBA-friendly) — **HOLD RESOLVIDO e APLICADA NO LOCAL em 2026-08-12**: a 77 foi **reescrita** com wrappers de compatibilidade de 30 dias (BLOCO 4.5), guard de idempotência no BLOCO 4 e **corpos chamadores reescritos para os nomes novos (BLOCO 4.6)** — `validar_anomalia_preco` chama `classificar_preco_anomalia` e `consolidar_produtos_duplicados` chama `relatorio_normalizacao_produtos`, tornando os wrappers puramente vestigiais (Fase 3 segura). **Evidência**: cross-references por nome no catálogo (`trg_valida_anomalia_preco`→`fn_classificar_preco_anomalia`; `fn_consolidar_produtos_duplicados`→`fn_relatorio_normalizacao`), zero refs em código vivo, validação em ROLLBACK (16 renames + 9 wrappers + 6 views, idempotente) e **validação pós-aplicação real no local**: 10 nomes novos + 9 wrappers, MV intacta (177.485), 0 nulo/CINZA, **trigger em runtime capturou preço anômalo R$99.999** (rejeição id 17841), 2ª execução no-op, smoke do guard PASS. **Pendência restante**: aplicar no Aiven (PRIMARY) após backup + aprovação do time — o local segue adiante como ambiente de homologação.
