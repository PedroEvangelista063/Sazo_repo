...
- **Migration 77 (nomenclatura)**: **APLICADA nos DOIS bancos (LOCAL + AIVEN, 2026-08-12)** — reescrita com **wrappers de compatibilidade (BLOCO 4.5, DEFAULTs replicados)** + guard de idempotência (BLOCO 4) + **corpos chamadores reescritos (BLOCO 4.6)** — wrappers vestigiais. Validado: 10 nomes novos, 9 wrappers, MV intacta (177.485), trigger em runtime, defaults 0-arg, 2ª execução no-op, smoke PASS; **consistência local×Aiven idêntica pós-sync**. **Fase 3 agendada**: `database/fase3_drop_compat_77.sql` (guard temporal 2026-09-30).
- **Migration 81 (Fase 2 — subconjunto seguro da nomenclatura)**: **APLICADA nos DOIS bancos (LOCAL + AIVEN, 2026-08-12)** — renomeia 4 funções sem refs de código (`normalizar_nome_produto`, `estatisticas_volatilidade_24m`, `calcular_semaforo_preco`, `encontrar_produto_pai`) + `baseline_sazonal_interpolado` + `vw_abastecimento_logistico`, com **7 corpos PL/pgSQL + 2 views reescritos na mesma transação** (gerados do catálogo, fidelidade 100%) e wrappers/views de compat (30 dias). Validado: MV 177.485, 0 nulo/CINZA, smoke PASS, `/fluxos` total=143, 2ª execução no-op. **Pendente**: `raw.coleta_bruta`/`controle_carga` + cadeia sandwich + `sp_calcular_sazonalidade`/`fator_kg_produto_uf` (exigem recriar procedures de produção).
