...
- **Migration 77 (nomenclatura)**: **APLICADA no LOCAL** (2026-08-12, backup `backup_schema_77_pre_*`) — reescrita com **wrappers de compatibilidade (BLOCO 4.5)** + guard de idempotência (BLOCO 4) + **corpos chamadores reescritos para nomes novos (BLOCO 4.6)** — wrappers ficam vestigiais, Fase 3 segura. Validado: 10 nomes novos, 9 wrappers, MV intacta (177.485), trigger em runtime capturando anômalo, 2ª execução no-op, smoke PASS. Pendência: aplicar no Aiven (PRIMARY) após aprovação.
