...
- **Migration 77 (nomenclatura)**: revisada → **HOLD RESOLVIDO** — a 77 foi reescrita com **wrappers de compatibilidade de 30 dias para as 9 funções renomeadas** (BLOCO 4.5) + guard de idempotência no BLOCO 4. Evidência: cross-references por nome no catálogo (`trg_valida_anomalia_preco`→`fn_classificar_preco_anomalia`; `fn_consolidar_produtos_duplicados`→`fn_relatorio_normalizacao`), zero refs em código vivo, e **validação em transação ROLLBACK** (16 renames + 9 wrappers + 6 views, idempotente na 2ª execução, wrappers delegando em runtime, trigger intacto). Pendência: aplicar local + Aiven após backup + aprovação.
