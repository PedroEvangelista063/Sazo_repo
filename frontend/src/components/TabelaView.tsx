...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  VERDE: { bg: 'bg-sazonal-verde-100 dark:bg-sazonal-verde-dark/30', text: 'text-sazonal-verde-700 dark:text-sazonal-verde-400', border: 'border-sazonal-verde-600' },
  AMARELO: { bg: 'bg-sazonal-amarelo-100 dark:bg-sazonal-amarelo-dark/30', text: 'text-sazonal-amarelo-700 dark:text-sazonal-amarelo-400', border: 'border-sazonal-amarelo-600' },
  VERMELHO: { bg: 'bg-sazonal-vermelho-100 dark:bg-sazonal-vermelho-dark/30', text: 'text-sazonal-vermelho-700 dark:text-sazonal-vermelho-400', border: 'border-sazonal-vermelho-600' },
}

const TENDENCIA_ICONS: Record<string, React.ReactNode> = {
  QUEDA: <ChevronDown className="w-4 h-4 text-green-600 dark:text-green-400" />,
  ALTA: <ChevronUp className="w-4 h-4 text-red-600 dark:text-red-400" />,
  ESTAVEL: <ChevronsUpDown className="w-4 h-4 text-gray-500 dark:text-gray-400" />,
}
...
export function TabelaView({ uf, categoria, ano, mes, onSelectProduto, selectedProduto }: TabelaViewProps) {
  ...
  const columns = useMemo<ColumnDef<ProdutoComPreco>[]>(() => [
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        return (
          <span className={cn('font-mono font-medium', isPositive ? 'text-red-600' : 'text-green-600')}>
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        const colors = STATUS_COLORS[status] ?? STATUS_COLORS.VERDE
        return (
          <Badge variant="secondary" className={cn(colors.bg, colors.text, colors.border, 'capitalize')}>
            {status.toLowerCase()}
          </Badge>
        )
      },
    ...
    },
    {
      ...
      cell: (info) => (info.getValue() as boolean ? <Badge variant="outline" className="text-xs">Sim</Badge> : '—'),
      size: 80,
    },
    {
        ...
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="h-9 w-64 pl-9 pr-3 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm outline-none focus:ring-2 focus:ring-sazonal-verde-600"
            />
          </div>
          <Button
            variant="outline"
