#!/usr/bin/env python3
"""Restore remaining empty tables to Supabase"""
import subprocess
import os
import json
import sys
...
def restore_table(schema, table):
    ...
    if not remote_cols:
        print(f"  ⚠️  {schema}.{table}: no remote columns found")
        return 0
    ...
    for i in range(0, len(rows), chunk_size):
        ...
        if r.returncode == 0:
            ...
            err = r.stderr.decode('utf-8', errors='replace')[:100] if r.stderr else "error"
...
def main():
    print("=" * 50)
    print("RESTORE REMAINING TABLES")
    print("=" * 50)
    total = 0
    for schema, table in TABLES_TO_RESTORE:
        print(f"\n📦 {schema}.{table}")
