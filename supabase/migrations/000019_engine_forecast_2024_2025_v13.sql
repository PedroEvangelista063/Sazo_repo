-- ============================================================================
-- QUERO COMPRAR — Migration 000019: Engine Forecast V13 — Âncora 2024 + Margem 2025
...
--     1. Rebuild mart.sazonalidade_baseline_24_25 (moda do status_cor real 2024/2025)
...
--     4. staging.sp_calcular_forecast_2026_v13() lê a baseline ponderada e
--        projeta os meses de 2026 sem dado real, PRESERVANDO linhas reais
--        (is_forecast = FALSE) no ON CONFLICT
--     5. sp_executar_carga_completa() passa a invocar a engine V13
--     6. fn_br_nacional_sazonalidade() expõe forecast_method + calculado_em
--
-- COMPATIBILIDADE COM A CADEIA:
--   - 000005 define UNIQUE (id_produto, id_localidade, ano, mes) em
--     mart.sazonalidade_produto → ON CONFLICT usa exatamente essa chave.
--   - 000013 adiciona categoria_b2c/id_categoria em dim_produto e as
--     colunas baseline_confianca/forecast_method/usou_fallback_12m/
--     preco_estimado em sazonalidade_produto.
--   - 000018 adiciona o método 'LOCF_SINGLE_MONTH' ao chk_forecast_method;
--     esta migration PRESERVA esse valor na nova constraint (senão o
--     ADD CONSTRAINT falharia com linhas LOCF_SINGLE_MONTH existentes).
--   - MV vw_api_produtos_sazonalidade tem índice único idx_vw_sazonalidade_unico
--     (000013) → REFRESH CONCURRENTLY é seguro.
--
...
BEGIN
    ...
    RAISE NOTICE '[019] Baseline 24_25 reconstruída: % linhas', v_n;
END;
$$;
...
BEGIN
    FOR v_mes IN 1..12 LOOP
        ...
        RAISE NOTICE '[019] Lote mês %: % linhas', v_mes, v_lote;
    END LOOP;
END;
$$;
...
BEGIN
    ...
    RAISE NOTICE '[019] Baseline ponderada: % linhas (cinzas: %)', v_total, v_gray;
END;
$$;
...
-- ============================================================================
-- Mantém o conjunto atual EXATAMENTE (000018 inclui LOCF_SINGLE_MONTH) +
-- os 3 novos métodos V13. A constraint preserva a permissão de NULL
-- (NULL = dado real).
...
ALTER TABLE mart.sazonalidade_produto
    ...
    CHECK (forecast_method IS NULL
           OR forecast_method IN (
               ...
               'PROXY_HIERARQUICO',
               'LOCF_SINGLE_MONTH',        -- ← preservado da 000018
...
-- scripts/restore/fix_supabase_schema.sql; esta seção garante o mesmo no
-- chain de migrations (idempotente: lookup genérico + DROP + ADD, padrão
-- da migration 19).
...
BEGIN
    ...
    --    NOTA: na cadeia de migrations Supabase a proc sp_project_sandwich_prices_2026
    --    NÃO existe (é aplicada via database/51/40 no banco vivo), por isso o guard
    --    EXISTS — aplicado em AMBOS os arquivos (000019 e database/62), seguindo o
    --    mesmo padrão das etapas 1-3.
    SELECT EXISTS (
        SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
